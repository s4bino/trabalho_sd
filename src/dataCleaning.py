...
def dataCleaning(url):
    ...
    try:
        ...
        result = subprocess.run(['curl', '-o', pageCompras, url], check=True, text=True, capture_output=True)
        if result.returncode != 0:
            raise RuntimeError("Falha ao executar o comando curl.")
...
if __name__ == '__main__':
    intervalo = 5  # Intervalo de tempo entre execuções em segundos
